...
import pandas as pd
import plotly.graph_objects as go # 新增圖表庫
import requests
from bs4 import BeautifulSoup

# --- 網頁配置 ---
st.set_page_config(page_title="小白股票診療室 Pro", layout="wide") # 改為寬版顯示
...
def get_stock_name(symbol):
    try:
        ...
        name = soup.find('h1').text.split(' ')[0] # 抓取中文名稱
        return name
...
if analyze_btn or stock_id:
    ...
    with st.spinner(f'正在讀取 {stock_id} 的大數據...'):
        try:
            ...
            st.error(f"看診發生錯誤，請確認網路或股號：{e}")

# --- 底部 ---
st.sidebar.markdown("---")
st.sidebar.caption("小白股票診療室 v2.2 | 數據僅供參考，投資請謹慎")
